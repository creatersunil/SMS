...
@ViewChild('childModal') childModal: ModalDirective;
 ...
 loadData() {
       ...
       this.dbService.query('mstcountry','',queryFilters).subscribe((result)=>{
           ...
           this.log.consoleLog(this.countries);
...
onCreateDialogConfirm(dialog,existdialog){
  this.log.consoleLog(JSON.stringify(this.event.newData));
   this.event.confirm.resolve();
      ...
      {
       this.log.consoleLog(JSON.stringify(this.event.newData));
       this.dbService.insert('mstcountry',this.event.newData).subscribe((data)=>this.log.consoleLog(data));
       dialog.hide();  
    }
      else{
          dialog.hide();
...
onEditeDialogConfirm(dialog,existdialog){
    ...
    if(!this.onEditValidate(this.event))
      ...
      this.log.consoleLog(JSON.stringify(this.event.newData));
     var queryFilters = new URLSearchParams();
       queryFilters.set('filter','intCountryId ='+this.event.newData.intCountryId);

    this.dbService.update('mstcountry',this.event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
     dialog.hide();  
    }
      else{
        dialog.hide();
...
onCreateDialogReject(dialog){
  ...
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //    var queryFilters = new URLSearchParams();
  //      queryFilters.set('filter','intCountryId ='+event.newData.intCountryId);

  //   this.dbService.update('mstcountry',event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
  //     }
  ...
  //     {
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //      this.dbService.insert('mstcountry',event.newData).subscribe((data)=>this.log.consoleLog(data));
  //     }
  //     else{
...
validateData(event):Boolean
{
  ...
  this.log.consoleLog(event.newData.txtCountryName.trim().length);
if(event.newData.txtCountryName.trim().length<1){
 return true;
}
else {
  this.log.consoleLog('validateFunction');
     return this.countries.find((countries) => {
       this.log.consoleLog(countries.txtCountryName.toUpperCase() === event.newData.txtCountryName.trim().toUpperCase());
       this.log.consoleLog('validateFunction');
                  return (countries.txtCountryName.toUpperCase().trim() === event.newData.txtCountryName.trim().toUpperCase()); 
...
if(event.newData.txtCountryName.trim().length<1){
  ...
  this.log.consoleLog('else');
    return this.countries.find((countries) => {
               this.log.consoleLog((countries.txtCountryName.toUpperCase()===event.newData.txtCountryName.trim().toUpperCase()));
                return((countries.txtCountryName.toUpperCase()===event.newData.txtCountryName.trim().toUpperCase()) );
