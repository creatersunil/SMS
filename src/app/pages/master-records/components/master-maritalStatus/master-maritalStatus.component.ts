...
import {MaritalStatus} from './master-maritalStatus';
...
@Component({
 ...
 styleUrls: ['./modals.scss'],
  templateUrl: './master-maritalStatus.html',
  providers:[DbService,Logs]
})
export class MasterMaritalStatusComponent implements OnInit {
 ...
 loadData() {
       ...
       this.dbService.query('mstmaritalstatus','',queryFilters).subscribe((result)=>{
            ...
            var data: any = result.json();            
              data.resource.forEach((maritalStatus) => {
                    this.maritalStatus.push(MaritalStatus.fromJson(maritalStatus));
                    this.log.consoleLog(this.maritalStatus.length);
                });
           this.log.consoleLog(this.maritalStatus);
             this.source.load(this.maritalStatus);
...
onCreateDialogConfirm(dialog,existdialog){
  this.log.consoleLog(JSON.stringify(this.event.newData));
   this.event.confirm.resolve();
      ...
      {
       this.log.consoleLog(JSON.stringify(this.event.newData));
       this.dbService.insert('mstmaritalstatus',this.event.newData).subscribe((data)=>this.log.consoleLog(data));
       dialog.hide();  
    }
      else{
          dialog.hide();
    ...
    if(!this.onEditValidate(this.event))
      {
       this.log.consoleLog(JSON.stringify(this.event.newData));
     var queryFilters = new URLSearchParams();
       queryFilters.set('filter','intMaritalStatusId ='+this.event.newData.intMaritalStatusId);

    this.dbService.update('mstmaritalstatus',this.event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
     dialog.hide();  
    }
      else{
        dialog.hide();
...
onCreateDialogReject(dialog){
  ...
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //    var queryFilters = new URLSearchParams();
  //      queryFilters.set('filter','intMaritalStatusId ='+event.newData.intMaritalStatusId);

  //   this.dbService.update('mstmaritalstatus',event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
  //     }
  ...
  //     {
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //      this.dbService.insert('mstmaritalstatus',event.newData).subscribe((data)=>this.log.consoleLog(data));
  //     }
  //     else{
...
validateData(event):Boolean
{
  ...
  this.log.consoleLog(event.newData.txtMaritalStatus.trim().length);
if(event.newData.txtMaritalStatus.trim().length<1){
 return true;
}
else {
  this.log.consoleLog('validateFunction');
     return this.maritalStatus.find((maritalStatus) => {
       this.log.consoleLog(maritalStatus.txtMaritalStatus.toUpperCase() === event.newData.txtMaritalStatus.trim().toUpperCase());
       this.log.consoleLog('validateFunction');
                  return ((maritalStatus.txtMaritalStatus.toUpperCase().trim() === event.newData.txtMaritalStatus.trim().toUpperCase())  ); 
...
if(event.newData.txtMaritalStatus.trim().length<1){
  ...
  this.log.consoleLog('else');
    return this.maritalStatus.find((maritalStatus) => {
               this.log.consoleLog((maritalStatus.txtMaritalStatus.toUpperCase()===event.newData.txtMaritalStatus.trim().toUpperCase()));
                return((maritalStatus.txtMaritalStatus.toUpperCase()===event.newData.txtMaritalStatus.trim().toUpperCase())  );
