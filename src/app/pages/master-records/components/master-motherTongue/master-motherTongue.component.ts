...
@Component({
  selector: 'motherTongue',
 styleUrls: ['./modals.scss'],
  templateUrl: './master-motherTongue.html',
  providers:[DbService,Logs]
})
export class MasterMotherTongueComponent implements OnInit {
 ...
 loadData() {
       ...
       this.dbService.query('mstmothertongue','',queryFilters).subscribe((result)=>{
            ...
            var data: any = result.json();            
              data.resource.forEach((motherTongue) => {
                    this.motherTongue.push(MotherTongue.fromJson(motherTongue));
                    this.log.consoleLog(this.motherTongue.length);
                });
           this.log.consoleLog(this.motherTongue);
             this.source.load(this.motherTongue);
...
onCreateDialogConfirm(dialog,existdialog){
  this.log.consoleLog(JSON.stringify(this.event.newData));
   this.event.confirm.resolve();
      ...
      {
       this.log.consoleLog(JSON.stringify(this.event.newData));
        this.dbService.insert('mstmothertongue',this.event.newData).subscribe((data)=>this.log.consoleLog(data));
       dialog.hide();  
    }
      else{
          dialog.hide();
    ...
    if(!this.onEditValidate(this.event))
      {
       this.log.consoleLog(JSON.stringify(this.event.newData));
     var queryFilters = new URLSearchParams();
       queryFilters.set('filter','intmothertongueId ='+this.event.newData.intmothertongueId);

    this.dbService.update('mstmothertongue',this.event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
     dialog.hide();  
    }
      else{
        dialog.hide();
...
onCreateDialogReject(dialog){
  ...
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //    var queryFilters = new URLSearchParams();
  //      queryFilters.set('filter','intmothertongueId ='+event.newData.intmothertongueId);

  //   this.dbService.update('mstmothertongue',event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
  //     }
  ...
  //     {
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //      this.dbService.insert('mstmothertongue',event.newData).subscribe((data)=>this.log.consoleLog(data));
  //     }
  //     else{
...
validateData(event):Boolean
{
  ...
  this.log.consoleLog(event.newData.txtmothertongue.trim().length);
if(event.newData.txtmothertongue.trim().length<1){
 return true;
}
else {
  this.log.consoleLog('validateFunction');
     return this.motherTongue.find((motherTongue) => {
       this.log.consoleLog(motherTongue.txtmothertongue.toUpperCase() === event.newData.txtmothertongue.trim().toUpperCase());
       this.log.consoleLog('validateFunction');
                  return (motherTongue.txtmothertongue.toUpperCase().trim() === event.newData.txtmothertongue.trim().toUpperCase()); 
...
if(event.newData.txtmothertongue.trim().length<1){
  ...
  this.log.consoleLog('else');
    return this.motherTongue.find((motherTongue) => {
               this.log.consoleLog((motherTongue.txtmothertongue.toUpperCase()===event.newData.txtmothertongue.trim().toUpperCase()));
                return((motherTongue.txtmothertongue.toUpperCase()===event.newData.txtmothertongue.trim().toUpperCase()) );
