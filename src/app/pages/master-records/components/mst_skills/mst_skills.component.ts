...
import {MstSkillsType} from './mst_skills';
import { LocalDataSource } from 'ng2-smart-table';
import {DbService } from '../../../../services';
import { FormGroup ,FormControl, Validators,FormBuilder,FormArray} from '@angular/forms';
import 'style-loader!./mst_skills.scss';
...
@Component({
 ...
 styleUrls: ['./modals.scss'],
  templateUrl: './mst_skills.html',
  providers:[DbService,Logs]
})
export class MstSkills implements OnInit {
       ...
       this.dbService.query('mst_skills','',queryFilters).subscribe((result)=>{
            ...
            var data: any = result.json();            
              data.resource.forEach((skills) => {
                    this.skills.push(MstSkillsType.fromJson(skills));
                    this.log.consoleLog(this.skills.length);
                });
           this.log.consoleLog(this.skills);
...
onCreateDialogConfirm(dialog,existdialog){
  this.log.consoleLog(JSON.stringify(this.event.newData));
   this.event.confirm.resolve();
      ...
      {
       this.log.consoleLog(JSON.stringify(this.event.newData));
      this.dbService.insert('mst_skills',this.event.newData).subscribe((data)=>this.log.consoleLog(data));
       dialog.hide();  
    }
      else{
          dialog.hide();
    ...
    if(!this.onEditValidate(this.event))
      {
       this.log.consoleLog(JSON.stringify(this.event.newData));
     var queryFilters = new URLSearchParams();
      queryFilters.set('filter','skillId ='+this.event.newData.skillId);

    this.dbService.update('mst_skills',this.event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
     dialog.hide();  
    }
      else{
        dialog.hide();
...
onCreateDialogReject(dialog){
  ...
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //    var queryFilters = new URLSearchParams();
  //      queryFilters.set('filter','skillId ='+event.newData.skillId);

  //   this.dbService.update('mst_skills',event.newData,queryFilters).subscribe((data)=>this.log.consoleLog(data));
  //     }
  ...
  //     {
  //      this.log.consoleLog(JSON.stringify(event.newData));
  //      this.dbService.insert('mst_skills',event.newData).subscribe((data)=>this.log.consoleLog(data));
  //     }
  //     else{
...
validateData(event):Boolean
{
  ...
  this.log.consoleLog(event.newData.skill_name.trim().length);
if(event.newData.skill_name.trim().length<1){
 return true;
}
else {
  this.log.consoleLog('validateFunction');
     return this.skills.find((skill) => {
       this.log.consoleLog(skill.skill_name.toUpperCase() === event.newData.skill_name.trim().toUpperCase());
       this.log.consoleLog('validateFunction');
                  return (skill.skill_name.toUpperCase() === event.newData.skill_name.trim().toUpperCase()); 
